...
#'   printed
#' @author Bowei Wei \email{wbwlazycat@126.com}
      ...
      if (current.status == 0) {
        ...
        if (current.mat == -1) {
          ...
          repeat{
            ...
            x = l$x
            y = l$y
            #if(x>33.7 & x<65.5 & y>32 & y<48){restart()}
...
#function for read the result from whether restart
result <- function(a) {
  a <- as.character(a)
  list(a=a)
}
#result <- function()  {
#  a <- (readline("Type Y or N: \n"))
#  a <- as.character(a)
#  list(a=a)
#}


#`Feed back functions for the answer of restart
answer <- function (a)  {
  ans <- result(a)
  Check <- c("YES", "NO")
  if (is.null(ans$a)) {
